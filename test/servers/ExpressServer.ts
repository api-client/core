...
export class ExpressServer {
  ...
  async startHttps(port?: number): Promise<number> {
    ...
    const key = await readFile(path.join('test', 'runtime', 'http-engine', 'certs', 'privkey.pem'));
    const cert = await readFile(path.join('test', 'runtime', 'http-engine', 'certs', 'fullchain.pem'));
