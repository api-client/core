...
describe('ArcPouchTransformer', () => {
  ...
  describe('PouchDB export system', () => {
    let exportObject: ArcExportObject;
    let jsonData: any;
    before(async () => {
      const response = await DataTestHelper.getFile('arc-pouchdb.json');
      jsonData = JSON.parse(response);
      const transformer = new ArcPouchTransformer(jsonData);
      exportObject = await transformer.transform();
    });

    it('does not process saved requests data', async () => {
      assert.typeOf(exportObject.requests, 'array', 'created requests array');
      assert.equal(exportObject.requests.length, jsonData.requests.length, 'has the same number of requests');
      const [srcRequest] = exportObject.requests;
      const [trgRequest] = jsonData.requests;
      ...
      assert.equal(trgRequest.kind, 'ARC#RequestData', 'the kind is set');
    ...
    it('does not process the history requests data', async () => {
      assert.deepEqual(exportObject.history, jsonData.history);
    });

    it('does not process the ws url data', async () => {
      assert.deepEqual(exportObject.websocketurlhistory, jsonData.websocketurlhistory);
    });

    it('does not process the url history data', async () => {
      assert.deepEqual(exportObject.urlhistory, jsonData.urlhistory);
    });

    it('does not process the url history data', async () => {
      assert.deepEqual(exportObject.variables, jsonData.variables);
    });

    it('does not process the authorization data', async () => {
      assert.deepEqual(exportObject.authdata, jsonData.authdata);
    });

    it('does not process the host rules data', async () => {
      assert.deepEqual(exportObject.hostrules, jsonData.hostrules);
    });
  });
});
